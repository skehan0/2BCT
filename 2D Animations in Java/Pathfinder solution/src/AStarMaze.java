...
// 2: animate game objects
			if (isGameRunning) {
				...
				if (player.move(map)) // player (potentially) moves every frame
					badguy.reCalcPath(map,player.x,player.y);// player moved so badguy recalls path
...
// toggle the state of the cell
		map[x][y] = !map[x][y];
		if (isGameRunning)
			badguy.reCalcPath(map,player.x,player.y); // maze changed so badguy recalcs path 3
