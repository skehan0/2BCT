    ...
    public static void main(String[] args) {
        ...
        if (args != null && args.length > 0) { // Check for <input> value
            ...
            if (start.length() != 8) {
                ...
                for (int k = 0; k < 10; k++) { // for loop to loop through the array
                    start = args[k];

                    for (i = 0; i < 10000; i++) { // iterate 10000 times
                        res = hashFunction(start); // call hash function
                        start = reductionFunction(res, i); // calling reduction hash function

                        // part 2

                        if (res == 895210601874431214L) { // finding if hash values match to the start values
